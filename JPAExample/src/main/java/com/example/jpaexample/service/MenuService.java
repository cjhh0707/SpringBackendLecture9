...
import com.example.jpaexample.domain.Category;
import com.example.jpaexample.domain.Menu;
import com.example.jpaexample.dto.MenuResponse;
import com.example.jpaexample.repository.CategoryRepository;
...
//        return repository.findByNameContaining(keyword).stream()
  ...
  public Slice<MenuResponse> findMenusSliceByCategoryAndMinPrice(
    ...
    return repository.findSliceByCategoryNameAndPriceGreaterThanEqual(categoryName, minPrice,
            pageable)
        .map(
            m -> new MenuResponse(m.getId(), m.getName(), m.getPrice(), m.getCategory().getName()));
  }
  ...
  @Transactional
  public void txCreateAndIncreaseWithRollback(String categoryName, String newMenuName,
      int newMenuPrice, int delta) {
    ...
    throw new RuntimeException("Force Exception(check Rollback)");
  }
  ...
  public int readTwicePrice(Long menuId) throws Exception {
    ...
    Thread.sleep(10000); // 5초 동안 다른 요청으로 가격 변경(커밋)하도록 시간 벌기
